...
class ReleaseDateConverter:
	...
	def clean_release_date(self, raw_date):
		...
		return str(raw_date.capitalize().encode('ascii', 'ignore').decode('utf-8'))
	...
	def format_date(self, coming_soon, raw_date, status, tba):
		...
		if tba == 0:
				...
				try:
					release_date = datetime.strptime(raw_date, '%m/%d/%Y').date()
					return release_date
				except ValueError as ve:
					print (ve)
		...
		if int(year) == int(year_now) and month == 'dec':
			year = str(int(year) + 1)
		...
		if coming_soon is True or status==5:
